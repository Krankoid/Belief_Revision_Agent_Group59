...
class BeliefBase:
    ...
    def revise(self, formula, weight):
        # if is_entailed(formula):
        #     self.belief_base.add(self.belief_base, formula)
        #     return
    ...
    def _remainder_sets(self, formula):
        ...
        for belief in self.belief_base:
            if len(self.belief_base) > 2:
                candidate_remainder = self.belief_base - {belief}
                print("candidate: ", candidate_remainder)

                if not is_entailed(candidate_remainder, formula):
...
if __name__ == "__main__":
    bb = BeliefBase()
    bb.belief_base = {sympify("p"), sympify("q")}
    bb.belief_weights = {sympify("p"): 0.8, sympify("q"): 0.5}
